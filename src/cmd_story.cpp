  ...
  return CommandSpec{
      ...
      {SubcommandSpec{
       ...
       SubcommandSpec{
           "ls",
           "[-C|--chat <id>] [-n|--limit <N>]",
           "List active stories for self or specific channel",
           {OptionSpec{"-C",
                       "--chat",
                       "<id>",
                       "Target chat or channel ID (default: personal account)",
...
App::cmd_story_ls(const std::vector<std::string> &args) {
  ...
  if (!res) {
    return std::unexpected("Failed to fetch active stories: " + res.error());
  }
  ...
  if (stories_array.empty()) {
    std::cout << "No active stories found for chat ID " << target_chat_id
              << ".\n";
    return 0;
  }

  size_t count =
      std::min(stories_array.size(), static_cast<size_t>(opts.limit));
  std::cout << std::format("Found {} active story/stories:\n\n", count);
  std::cout << std::format("{:<10} {:<20} {:<10} {:<15} {:<30}\n", "STORY_ID",
                           "DATE", "PINNED", "CONTENT_TYPE", "CAPTION");
  std::cout << std::string(85, '-') << "\n";

  for (size_t i = 0; i < count; ++i) {
    const auto &story_obj = stories_array[i];
    int64_t sid = story_obj.get_int("id").value_or(0);
    int64_t date_ts = story_obj.get_int("date").value_or(0);
    bool is_pinned = story_obj.get_bool("is_pinned").value_or(false);

    std::string content_type = "unknown";
    auto content_obj = story_obj.get_object("content");
    if (content_obj) {
    ...
    auto caption_obj = story_obj.get_object("caption");
    if (caption_obj) {
      caption_text = caption_obj->get_string("text").value_or("");
    }
    if (caption_text.size() > 28) {
      caption_text = caption_text.substr(0, 25) + "...";
    }

    std::cout << std::format("{:<10} {:<20} {:<10} {:<15} {:<30}\n", sid,
                             date_ts, is_pinned ? "Yes" : "No", content_type,
                             caption_text);
  }
